...
Para establecer un modelo lineal que permita estimar el precio, uso como variable dependiente el logaritmo del precio, a efectos de obtener mejores modelos, y para asegurarse de que las estimaciones sean positivas. Acto seguido, creo primero el modelo lineal saturado, usando todas las variables salvo latitud, longitud, precio, la variable registo creada en en el ejercicio anterior, y el tipo de habitación (ya que todos los pisos son de la misma categoría: "Entire home/apt").
...
Parto nuevamente de la misma fórmula que el modelo lineal depurado, solo que ahora usamos un modelo SEM, donde se toman en cuenta los efectos de los rezagos espaciales de los errores, los cuales son multiplicados por el coeficiente lambda, el cual se obtiene a continuación.
...
```
El modelo SAR es el que presenta mejor coeficiente de determinación de los 3 modelos analizados. No obstante, cabe mencionar que el modelo SE presenta un mejor ajuste que el LM depurado.
...
Primero que nada, se debe definir el ancho espacial óptimo para ponderar el modelo.
...
Hay claramente una distinción entre oeste y este de Madrid. 