...
Primero que nada es necesario separar la base en train y test. Usaré 70% para train y 30% para test.

```{r separo en test y train}

#Fijo semilla
set.seed(123)

#hago muestreo
train <- sample(1:nrow(pisos_muestra),size = round(nrow(pisos_muestra)*0.7,0))

#Particiono en test y train
pisos_train <- pisos_muestra[train,]
pisos_test <- pisos_muestra[-train,]
...
#Hago el summary
summary(modelo_espacial_sar_CV)
```
```{r}
...
rownames(pisos_test_X) <- 1:nrow(pisos_test_X)


fitt<-solve(as.matrix(diag(nrow(pisos_test_X))-(as.numeric(modelo_espacial_sar_CV$rho)*nb_test_matrix )))
fitt2<-as.matrix(pisos_test_X) %*% as.matrix(modelo_espacial_sar_CV$coefficients[-1])+modelo_espacial_sar_CV$coefficients[1]
fit_final<-fitt %*% fitt2

```

```{r}
paste("residuos modelo SAR con CV",sum((fit_final-  pisos_test$logprice)**2))
```
